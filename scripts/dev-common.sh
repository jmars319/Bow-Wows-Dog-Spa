...
function browser_auto_open_enabled() {
  if [[ "${BOWWOW_SKIP_BROWSER_OPEN:-0}" == "1" ]]; then
    return 1
  fi
...
function open_url_in_system_browser() {
  local url=$1
  local platform
  platform=$(uname -s)
...
function reload_existing_browser_tab() {
  local url=$1
  ...
  local result
  result=$(osascript - "$url" "$alternate_url" <<'APPLESCRIPT'
on matchesTarget(candidateUrl, primaryUrl, alternateUrl)
  if candidateUrl is missing value then
    return false
  end if

  set candidateText to candidateUrl as text
  if candidateText starts with primaryUrl then
    return true
  end if

  if alternateUrl is not "" and candidateText starts with alternateUrl then
    return true
  end if
...
on reloadInChromium(browserName, primaryUrl, alternateUrl)
  tell application browserName
    repeat with w in windows
      repeat with tabIndex from 1 to (count of tabs of w)
        ...
        if my matchesTarget(candidateUrl, primaryUrl, alternateUrl) then
  ...
  return false
end reloadInChromium

on reloadInSafari(primaryUrl, alternateUrl)
    ...
    repeat with w in windows
      repeat with tabIndex from 1 to (count of tabs of w)
        ...
        if my matchesTarget(candidateUrl, primaryUrl, alternateUrl) then
          ...
          set index of w to 1
          activate
          do JavaScript "window.location.reload();" in current tab of w
          return true
        end if
      end repeat
    end repeat
...
on run argv
  ...
  set chromiumBrowsers to {"Google Chrome", "Brave Browser", "Microsoft Edge", "Arc"}

  repeat with browserName in chromiumBrowsers
    try
      if application (browserName as text) is running then
        if my reloadInChromium(browserName as text, primaryUrl, alternateUrl) then
          return "reloaded:" & (browserName as text)
        end if
      end if
    end try
  end repeat
  ...
  try
    if application "Safari" is running then
      if my reloadInSafari(primaryUrl, alternateUrl) then
        return "reloaded:Safari"
      end if
    end if
  end try
...
function ensure_browser_tab() {
  ...
  local reload_result
  if reload_result=$(reload_existing_browser_tab "$url" "$alternate_url"); then
    local browser_name=${reload_result#reloaded:}
    log_status "browser" "success" "Reloaded existing tab in ${browser_name}"
    return 0
  fi

  if open_url_in_system_browser "$url"; then
    log_status "browser" "success" "Opened ${url}"
    return 0
  fi

  log_status "browser" "warn" "Unable to open ${url} automatically"
  return 0
}
